...
class LeoORM:
    ...
    async def _save_bulk(self, instances):
        ...
        sql = (
            ...
            pk=pk,
            names=LazyStr(lambda: ', '.join(names)),
            values=LazyStr(lambda: ', '.join(
                '${}'.format(i) for i in range(1, len(names) + 1)
            )),
        )
        if len(instances) == 1:
            args = values[0]
            coro = self.conn.fetchval(sql, *args)
            first_obj.pk = await self._exec(coro, 'leoorm._save_one', sql, args)
            setattr(first_obj, pk, first_obj.pk)
            logger.debug('leoorm._save_one: returns %r', first_obj.pk)
            await self._call_post_save(model_class, [first_obj], True)
            return first_obj
        else:
            coro = self.conn.executemany(sql, values)
            await self._exec(coro, 'leoorm._save_many', sql)
    ...
    async def _exec(self, coro, name, sql, values=None):
        ...
        logger.debug(
            ...
            ms,
            sql,
            LazyStr(lambda: dict(enumerate(values, start=1)) if values else ''),
